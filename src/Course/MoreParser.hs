...
betweenCharTok left right = between (is left) (is right)
...
hex = do
    ...
    fromMaybeParser $ chr <$> readHex cs

fromMaybeParser :: Optional Char -> Parser Char
fromMaybeParser Empty = constantParser UnexpectedEof
fromMaybeParser (Full x) = valueParser x
...
sepby p sepp = sepby1 p sepp ||| valueParser Nil
...
-- True
eof :: Parser ()
eof = P (\input -> if isEmpty input then Result input () else UnexpectedString input)
...
betweenSepbyComma left right p = betweenCharTok left right $ sepby p (charTok ',')
