...
jsonChar = do 
    ...
    else if c == '"' then unexpectedCharParser '"'
         else return c
...
jsonObject = betweenSepbyComma '{' '}' ((,) <$>
    spaces *> jsonString <* spaces <* charTok ':' <* spaces <*> jsonValue)
    <* spaces
...
jsonValue = (const JsonNull <$> spaces *> jsonNull)
    ||| (const JsonTrue <$> spaces *> jsonTrue)
    ||| (const JsonFalse <$> spaces *> jsonFalse)
    ||| (JsonString <$> spaces *> jsonString)
    ||| (JsonRational <$> spaces *> jsonNumber)
