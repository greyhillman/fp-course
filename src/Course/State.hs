...
instance Functor (State s) where
  ...
  f <$> sa = State (\s -> fstApp f (runState sa s))
...
findM p (x:.xs) = help =<< p x
    where
        help True = pure $ Full x
        help False = findM p xs
...
firstRepeat xs = eval (findM (\x -> State (stateFunction x)) xs) S.empty
    where
        stateFunction :: Ord a => a -> S.Set a -> (Bool, S.Set a)
        stateFunction x set
            | S.member x set = (True, set)
            | otherwise      = (False, S.insert x set)
...
distinct xs = eval (filtering (\x -> State (stateFunction x)) xs) S.empty
    where
        stateFunction :: Ord a => a -> S.Set a -> (Bool, S.Set a)
        stateFunction x set
            | S.notMember x set = (True, S.insert x set)
            | otherwise      = (False, set)
...
isHappy n = contains 1 $ firstRepeat $ produce (toInteger . sum . map square . digits) n
    where
        square = join (*)
